    ...
    m_MaxCount(MaxCount)
{
    ...
    {
        m_MaxCount = 0;
        LogErr << "SemLite::SemLite(): Failed to initialize semaphore: "
               << NameGet() << std::endl;
    }
}
...
SemLite::~SemLite()
{
    ...
    if (m_Valid)
    {
        ...
        {
            LogErr << "SemLite::~SemLite(): Failed to destroy semaphore: "
                   << NameGet() << std::endl;
        }
    }
}
...
bool SemLite::Take(uint32_t Timeout)
{
    ...
    if (m_Valid)
    {
        ...
        {
            LogErr << "SemLite::Take(): Failed to take semaphore due to timeout: "
                   << NameGet() << std::endl;
        }

        if (result == WAIT_ABANDONED)
        {
            LogErr << "SemLite::Take(): Failed to take semaphore due to thread closing: "
                   << NameGet() << std::endl;
        }
    }
...
bool SemLite::Give()
{
    ...
    {
        LogErr << "SemLite::Release(): Failed to give semaphore: "
               << NameGet() << std::endl;
    }
...
#if 0       // (.)(.)  this risks releasing a blocked thread
    // check for instance validity
    if (m_Valid)
    {
        // Determine count.
